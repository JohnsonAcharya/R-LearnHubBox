# ============================================================
# Program: table_04_vital_signs.R
#
# Purpose:
#   Summarize observed vital-sign values and change from
...
#
# Outputs:
#   output/tables/table_04_vital_signs.csv
#   output/tables/table_04_vital_signs.md
#
...
stopifnot(all(required %in% names(advs)))
stopifnot(!anyDuplicated(advs[c("USUBJID", "ASEQ")]))
stopifnot(all(advs$STUDYID == "ABC101"))
stopifnot(all(advs$SAFFL %in% c("Y", "N")))

safety <- advs[advs$SAFFL == "Y", ]
...
param_order <- c("SYSBP", "DIABP", "PULSE", "TEMP", "WEIGHT")
visit_order <- 1:5
...
for (parameter in param_order) {
  for (visit_number in visit_order) {
    ...
    for (measure in measures) {
      ...
      row <- data.frame(
        PARAMCD = parameter,
        `Vital-sign test` = parameter_info[[parameter]]$label,
...
write.csv(table, "output/tables/table_04_vital_signs.csv",
          row.names = FALSE, na = "")
...
writeLines(markdown, "output/tables/table_04_vital_signs.md")
